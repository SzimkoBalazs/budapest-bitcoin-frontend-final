...
const PlusMinusBtn = ({ isPlus, onClick, isDisabled }) => {
  ...
  return (
    <button
      ...
      className={cln(
        isPlus ? 'bg-primary-500' : 'bg-neutral-100',
        !isDisabled && 'active:shadow-none active:translate-y-[6px] cursor-pointer',
        'w-[64px] relative flex h-[42px] items-center rounded-[10px] border-2 border-black shadow-[0px_6px_0px_0px_#000] transition-shadow duration-100 justify-center',
      )}
    >
      <h5
        className={cln(isPlus ? 'text-white' : 'text-black', 'text-[24px]')}
