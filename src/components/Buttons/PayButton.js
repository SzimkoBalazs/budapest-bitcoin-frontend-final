...
const PayButton = ({ type, onClick, style, text, disabled, isCardPayment }) => {
  return (
    <button
      type={type}
      onClick={onClick}
      className={cln(
        ...
        'flex h-[50px] flex-col min-w-[136px] w-fit px-[24px] justify-center items-center rounded-[40px] transition-all duration-100',
      )}
      style={style}
      disabled={disabled}
    >
