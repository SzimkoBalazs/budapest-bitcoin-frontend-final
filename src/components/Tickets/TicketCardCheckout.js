...
const priceWithSpace = (number) => {
  ...
  return (
    ...
    <div
        ref={ticketRef}
      className="flex relative flex-row min-w-[288px] w-full items-center justify-between rounded-[10px] border-[2px]  bg-black"
      style={{
        borderColor: numberOfTickets === 0 ? 'white' : '#FFAE0B',
        boxShadow: numberOfTickets === 0 ? `1px 1px 0px #FFF, 2px 2px 0px #FFF, 3px 3px 0px #FFF, 4px 4px 0px #FFF, 5px 5px 0px #FFF` : `1px 1px 0px #FFAE0B, 2px 2px 0px #FFAE0B, 3px 3px 0px #FFAE0B, 4px 4px 0px #FFAE0B, 5px 5px 0px #FFAE0B, 7px 7px 7px 2px rgba(255, 174, 11, 0.15)`,
      }}
    >
      {/*INSIDE CONTAINER FOR BOTH TEXTS*/}
      <div className="flex gap-y-8 flex-col h-full p-3 lg:p-4 justify-between items-end"  style={{width:'calc( 100% - 48px )'}}>
        ...
        <div className="flex flex-row w-full justify-between">
          ...
          {/*INFO ICON*/}
          <button className="flex items-center justify-center w-6 h-6 rounded-2xl border-2 border-neutral-300" onClick={()=>setIsTicketInfoVisible(true)}>
            <span className="text-neutral-300 text-[14px] font-exo font-bold">
            i
          </span>
          </button>
          {isTicketInfoVisible && <TicketInfo details={details} name={name} onCloseClick={()=>setIsTicketInfoVisible(false)}/>}
           ...
           <div className="flex flex-row items-center gap-y-1">
            ...
            <div className="flex relative items-end justify-center gap-x-1">
              <h3 className="text-white text-[24px] lg:text-[34px] tracking-[2.4px]" style={{ fontWeight:800, lineHeight:'100%'}}>{priceWithSpace(price)}</h3>
              <h3 className="text-white right-[-36px] lg:right-[-50px] text-[16px] lg:text-[22px] mb-[2px] lg:mb-[4px]" style={{ fontWeight:400, lineHeight:'100%'}}>{'EUR'}</h3>
            </div>
          </div>
      </div>
      <div
      ...
      >
        <Image src={BarCodeUp} alt={'Barcode'} height={barcodeHeight} style={{height:barcodeHeight}}/>
      </div>
      {/*TICKET COUNTER BLUE BOX*/}
      {numberOfTickets > 0 && (
          <div
          className={cln(animate, "flex items-center justify-center w-[36px] h-[36px] absolute bottom-[-16px] left-[-8px] secondary-600 rounded-[6px] bg-secondary-600")}>
            <span className="font-exo text-white text-[24px] font-bold">
              {numberOfTickets}
            </span>
          </div>
      )}
